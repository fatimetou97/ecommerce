          ...
          <div class="card">
            <div class="card-header pb-0">
              <div class="d-flex align-items-center">
                <p class="mb-0">Edit attribute</p>
                <button class="btn btn-primary btn-sm ms-auto">Image</button>
              </div>
            </div>
            <div class="card-body">
              <p class="text-uppercase text-sm">attribute Fields</p>
              <form class="forms-sample" action="{{url('/web/edit_attribute/'.$attribute->id) }}" method="post">
                                  @csrf
             <input class="form-control" type="hidden" value={{$attribute->id}} name="id">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="example-text-input" class="form-control-label" >attribute Name</label>
                    <input class="form-control" required type="text" value={{$attribute->name}} name="name">
           @error('name')
